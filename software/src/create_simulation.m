function [reservoir, model] = create_simulation(C)
    ...
    % C is a cell array of the cleaned up input text file
    ...
    domain_shape = sscanf(C{contains(C, 'COORDINATE')}, 'COORDINATE %s');
    dykstra_parsons_coef = sscanf(C{contains(C, 'DP')}, 'DP %f');
