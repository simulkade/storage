...
\documentclass[a4paper,english]{extreport}
...
\usepackage{babel}
\begin{document}
\title{Physical and chemical energy storage in the abandoned offshore reservoirs}
\author{Ali A. Eftekhari}
\maketitle
\begin{abstract}
The existing and in development offshore windmills in the North Sea
will ensure the availability of abundant and cheap electricity in
the region, yet fail to address the intermittent nature of the wind
resources. The surplus electricity in the windy days and off peak
time can be converted to physical (compressed and hot fluids) or chemical
(synthetic fuels such as ammonia, methanol, or methane) forms and
stored in the reservoirs. The stored energy is extracted and converted
to electricity when there is a shortage of wind, or consumed as clean
fuels for transportation. This project aims to study the feasibility
of subsurface physical and chemical energy storage and extraction
as an alternative for the abandonment of the North Sea oil and gas
reservoirs. The feasibility and environmental impacts/benefits of
the following energy storage processes will be investigated: green
hydrogen, ammonia, methane, methanol, and compressed air. The main
focus points of the project are the investigation and quantification
of 

1- the importance of subsurface energy storage for the future energy
plans of Denmark.

2- the required subsurface and topside criteria for the operation
of the subsurface storage projects. 

3- the energy efficiency of different energy storage and extraction
processes.

The report is organized in four chapters. In chapter 1, the electricity
supply and demand of Denmark is analyzed and adjusted for the year
2050. The average surplus electricity is quantified and the time series
data for both surplus and shortage of electricity is presented. Based
on the presented data, the minimum efficiency requirement, the capacity
of the synthetic fuel production processes, and the minimum subsurface
volume requirement are quantified. In chapter 2 the energy efficiency
of the production of different energy storage media is presented.
In chapter 3, the energy efficiency of the subsurface energy storage
and subsequent production is quantified using a mathematical model
for the single phase single component compressible flow in porous
media. Finally, in chapter 4 the main findings of this report and
...
There are exceeding scientific evidence for the role of anthropological
carbon dioxide emission in an accelerated climate change, with potentially
disastrous -although uncertain- aftermath. This has increased the
societal pressure for a faster transition from hydrocarbon-based to
renewable-based energy. Besides these political and societal motivations,
the declining conventional oil resources has reduced the efficiency
of oil and gas recovery operations and increased the costs, leading
eventually to the cessation of production and abandonment of the fields.
The infrastructures, e.g. wells, platforms, topside equipment for
transport and processing, and transport pipelines, even though still
functional, will have to be removed as part of the abandonment operation. 

At the same time, the renewable energy resources will grow in many
areas of the world. In the North Sea region, several new windfarms
are planned or under construction, which will quadruple the capacity
of renewable electricity production. This capacity that is several
times more than the electricity demand in Denmark, is variable and
intermittent and thus will not replace the reliable fossil-fuel power
plants. The key factor in the transition to a fully renewable energy
infrastructure is a large scale energy storage that can act as a buffer
to provide a continuous power stream to the grids.

This project will investigate the feasibility of the alternative use
...
the promising technologies from a technical point of view. This report
...
\chapter{Chemical and physical energy storage}
...
studied. To produce a carbon-neutral fuel, the raw material for the
production of the synthetic fuels must come from the nature, i.e.
atmosphere and sea water, also known as the ``dead sate'' in the
thermodynamic literature. The dead state refers to the average composition
of the atmosphere, Earth's crust, and oceans that theoretically does
...
\section{Atmospheric nitrogen and carbon dioxide}
...
the process is energy intensive. Different values are reported in
the literature for the electricity consumption per unit mole of separated
nitrogen. These values depend on the heat integration of the process.
The largest value reported in the literature is around 15 kJ/mol air
distilled. In our simulation, we obtained a value of 32 kJ/mol, which
is perhaps due to the poor heat integration in the simplified simulations. 
...
\cite{sankaranarayanan_efficiency_2010}, i.e.
\[
...
exceeds 5\% \cite{keith_why_2009,mahmoudkhani_low-energy_2009}. This
means that the energy requirement for the separation of CO$_{2}$
...
of the art technology \cite{noauthor_direct_nodate}.
...
the footprint of the eqipment is between 0.5 to 2 km$^{2}$ for capturing
1 million ton of CO$_{2}$ per year \cite{noauthor_direct_2021,mcqueen_cost_2020}.
...
plant, if the capturing and conversion to fuel is performed in a close
cycle can also be carbon-neutral. However, these two sources are available
onshore and it is therefore easier to utilize them as close to the
source as possible.

The other source of carbon is the remaining oil and gas in the reservoirs,
which can be extracted by, e.g. insitu combustion. However, this carbon
source is not sustainable and we will not consider it. 
...
\begin{itemize}
\item Fitting the water treatment, electrolysis, and compression equipment
on a platform 
\item Pipeline integrity (for storage and gas transport) 
\item Physico-bio-chemical interactions of hydrogen with the formation fluids
and the chalk reservoir 
\item Capacity of reservoirs for storage of hydrogen and flow of hydrogen
in the reservoir 
\item Energy efficiency and economic value of the process 
\end{itemize}
In this research, the energy requirement of the process, the size
of the process equipment, and the flow of hydrogen in the chalk reservoirs
will be addressed. 
...
of Gibbs free energy \cite{rosen_energy_1995}. This reaction requires
39 kWh of electricity per kg hydrogen, but with the state of the art
technology the value is closer to 48 kWh/kg \cite{gardner_hydrogen_2009,ozarslan_large-scale_2012}. 
...
is recycled \cite{rosen_energy_1995}. Some extra moisture remains
in the hydrogen stream, which will be removed later in the intercoolers
between the compression stages \cite{rosen_energy_1995}. Electricity
must be provided as direct current (DC), which introduces a loss of
2.5\% for AC-DC conversion \cite{rosen_energy_1995}. Rosen assumes
an exergy efficiency of around 70\% for the electrolysis of water
for a mature electrolysis technology in 1995 (not considering other
losses). He does not give more information on other steps of the process,
e.g. water treatment and purification. 

There are several technologies to produce hydrogen from water using
electricity among which two are more mature and are available as commercial
packages, viz. alkaline electrolysis \cite{brauns_alkaline_2020}
and polymer electrolyte membrane cells (PEM) \cite{babic_critical_2017}.
...
\cite{granovskii_exergetic_2007}. An efficiency of 60\% to 70\% is
expected for these electrolysis processes, in which most of the energy
loss is converted to heat. The energy loss during the compression
...
\cite{ozarslan_large-scale_2012,zivar_underground_2020}. Due to the
high consumption of hydrogen, the petrochemical plants have been using
salt caverns to store large volumes of hydrogen in the subsurface
\cite{ozarslan_large-scale_2012}. Currently, the subsurface storage
of green hydrogen is being studied by several investigators in different
continents.
...
end of 2021 \cite{noauthor_orsted_nodate} that will produce up to
1000 kg/day of green hydrogen. These capacities, although an step
forward, will not address the GW scale energy shortage that will certainly
occur due to the intermittent nature of renewable electricity, as
was discussed in Chapter \ref{chap:Problem-definition}. 

The other issue that needs to be addressed here is the footprint of
the electrolysis equipment. We saw in Chapter \ref{chap:Problem-definition}
...
and requires 3 $\times$ 12 m$^{2}$ area. Therefore, a 500 MW electrolysis
plant that is made of 20 PEM cells is unlikely to to fit on a platform
with other equipment although it is not impossible. A better option
for the offshore hydrogen production is the installation of these
electrolysis cells onboard a ship designed for this purpose. Otherwise,
the onshore production of hydrogen seems to be the better option.
...
GW capacities\footnote{As an example, Dan F host 6,800 tonnes of facilities and provides
a power of 55 MW to the facilities. The highest point on the platform
is 132 m above water.}. For example, a typical 25 MW turbine that is designed for offshore
platforms and consumes a mixture of methane and hydrogen weighs approximately
250 tonnes. Therefore, up to several hundred MW of gas turbine capacity
can fit on a platform. But reaching to GW capacities can be a challenge.

\subsection{Methane}

Methanation process for the catalytic conversion of CO$_{2}$ and
CO to methane was first suggested by Sendersen and Sabatier \cite{bolt_critical_2020}.
...
methane is high \cite{loder_reaction_2020}. A selectivity of close
to 100\% is observed for catalytic methanation of CO$_{2}$ over nickel
catalyst at a temperature close to 300$^{o}$C. Although theoretically
the reaction reaches 100\% yield at room temperature, in practice
the reaction rate is too small and the methane yield is zero. A temperature
of at least 200$^{o}$C is needed to get the catalytic reaction started.
At temperatures above 500$^{o}$C, the reaction goes backward, i.e.
conversion of methane and water to hydrogen and CO$_{2}$. This reaction
is also known as methane reforming that is currently the main reaction
for the production of the so-called ``blue'' hydrogen, i.e. hydrogen
that is produced from a hydrocarbon. A schematic of the process for
the production of green methane is shown in Fig. \ref{fig:methane-process}.
In this process, the required hydrogen and carbon dioxide are produced
by the electrolysis of water and separation of atmospheric CO$_{2}$,
respectively. The carbon source can also be provided from other alternative
sources such as biomass or from the captured carbon dioxide from,
e.g. cement or steel industries. However, capturing CO$_{2}$ from
the atmosphere has the benefit of making the produced methane carbon-neutral
or so-called ``green''. The reactor pressure is 20-70 bar \cite{er-rbib_modelling_2013}
...
$i$, and $\eta_{j}$ {[}-{]} shows the exergetic efficiency \cite{eftekhari_exergy_2012}
...
673 K. By calculating the compression energy using the method described
in section \ref{subsec:Compression-exergy} and replacing other efficiency
...
to 500$^{o}$C \cite{elnashaie_simulation_1988,morud_analysis_1998}.
...
as is the case when carbon dioxide is used as a reactant \cite{elnashaie_simulation_1988}.

\begin{figure}[H]
\includegraphics[width=12cm]{haber\lyxdot gv}

\caption{\label{fig:Haber-process}Haber process for the production of ammonia}
\end{figure}
...
\end{equation}
 The reactor operates at 100 bar and 264$^{o}$C \cite{luyben_design_2010}.
...
methanol can be degraded by the subsurface microorganisms \cite{sousa_deep-subsurface_2018}.
...
\caption{A schematic of the methanol synthesis process}
\end{figure}


\section{Compressed air storage}

Compressed air storage is a relatively mature technology \cite{stottlemyre_preliminary_1978}.
In the US, large tanks are used for the compression of air during
the off-peak time to store cheap electricity in the form of high pressure
air. The air is later used to drive a turbine and produce electricity
during the peak time. This approach, although not energy efficient,
is economical due to the special model of electricity pricing that
encourages the consumers to shift their electricity demand to the
off-peak hours. The large scale storage of electricity in the subsurface
was first suggested as a method to reduce the waste energy of the
large fossil fuel power plants. Many of the power plants are designed
for a continuous operation and therefore their efficiency is significantly
reduced if they operate under the design condition. It is not convenient
to shut down the power plants when there is no consumer for the produced
electricity because the start up is time consuming, expensive, and
inefficient. Compressed air storage is one way of storing the surplus
energy of a power plant that operates at its design condition. The
large amount of stored energy can provide extra electricity during
the peak time to balance the demand. The same idea can be used with
the offshore windfarms, utilizing the abandoned gas fields as a storage
space.

Fig. \ref{fig:CA-schematic} shows an schematic of the compressed
air energy storage that is also similar to any other gas storage processes.
Air is injected in to the reservoir through a compressor. Since the
input electricity to the process is variable, the efficiency of the
injection compressor will vary since it does not always operate at
its design condition. The injected air (or hydrogen, methane) is produced
during the electricity shortage to flow through a gas turbine that
rotates a generator and produces electricity. If all the friction
losses in the reservoirs and in the well are ignored, the efficiency
of this process can be as high as 50\% (simply multiplying the efficiency
of the compressor and the gas turbine). The efficiency drops in the
dynamic operation due to the variable power input and also the friction
and heat losses in the reservoir. This is discussed in the next chapter.

\begin{figure}[H]
\includegraphics[width=14cm]{gas_storage_process}

\caption{\label{fig:CA-schematic}A block flow diagram of the subsurface gas
storage process and the important efficiency factors}
\end{figure}


\subsection{Compression exergy\label{subsec:Compression-exergy}}

The exergy of compressed air is calculated by
\[
ex\left(T,p\right)=\left(H\left(T,p\right)-H\left(T_{00},p_{00}\right)\right)-T_{00}\left(S\left(T,p\right)-S_{00}\left(T_{00},p_{00}\right)\right),
\]
where $T_{00}$ {[}288.15 K{]} and $p_{00}$ {[}101325 Pa{]} denote
the dead state temperature and pressure, respectively; $H$ {[}kJ/mol{]}
and $S$ {[}kJ/(mol.K){]} denote the enthalpy and entropy, respectively
that are calculated using the software CoolProp \cite{bell_coolprop_2013,bell_pure_2014}.

The power consumption for the compression of air in a multi-stage
compressor with interstage cooling is calculated in the following
steps. First, the number of stages that is required to compress a
gas stream from pressure $p_{in}$ to pressure $p_{out}$ is calculated
by
\[
n_{stage}=\frac{\ln\left(p_{out}/p_{in}\right)}{\ln r_{comp}},
\]
where $r_{comp}=p_{out}/p_{in}$ is the compression ratio for each
compressor. The number of compressors must be an integer number. Therefore,
we choose the floor of $n_{stage}$ as the number of compressors.
We choose a compression ratio of 3.5 here.
\begin{enumerate}
\item $p_{out}=r_{comp}p_{in}$
\item Calculate the entropy $S_{in}\left(p_{in},T_{in},x_{i}\right)$ and
enthalpy $H_{in}\left(p_{in},T_{in},x_{i}\right)$ of the input gas
stream using the physical property package CoolProp.
\item Calculate the output temperature $T_{out}$ {[}K{]} by solving the
following nonlinear equation: $S_{in}\left(p_{in},T_{in},x_{i}\right)-S_{out}\left(p_{out},T_{out},x_{i}\right)=0$.
\item Calculate the enthalpy of the output -compressed- gas stream $H_{out}\left(p_{out},T_{out},x_{i}\right)$
\item Calculate the isentropic compression (theoretical) work: $W_{th}=H_{out}-H_{in}$.
\end{enumerate}
The theoretical compression work can be converted to a practical value
by considering the efficiency factor of the compressor, its electrical
driver, and the efficiency factor for electricity transmission from
the windmill, $\eta_{comp}$, $\eta_{driver}$, and $\eta_{transmission}$
respectively, i.e.,
\[
W_{pr}=\frac{W_{th}}{\eta_{comp}\eta_{driver}\eta_{transmission}}.
\]
Typical values for these efficiency factors are shown in Table \ref{tab:Typical-values-etta}.

\begin{table}[H]
\caption{\label{tab:Typical-values-etta}Typical values for the efficiency
factors of compressors, electrical drivers, and power plants}

\begin{tabular}{|c|c|c|c|}
\hline 
Device & Compressor & Electrical driver & Electricity transmission\tabularnewline
\hline 
\hline 
Efficiency factor (\%) & 70 & 90 & 95\tabularnewline
\hline 
\end{tabular}
\end{table}


\chapter{Subsurface energy storage}
...
of the reservoir using an analytical solution \cite{hagoort_fundamentals_1988,al-hussainy_application_1966,al-hussainy_flow_1966}.
...
\section{Subsurface storage model}
...
software package FVTool \cite{eftekhari_fvtool_2015} in a two-dimensional
axisymmetric (i.e. cylindrical) coordinate. We used different permeability
fields, including higher permeability near the wellbore region. A
version with the possibility of adding fracture is also implemented.
The permeability field that represents Harald West field with an stimulated
near-well zone is shown in Fig. \ref{fig:Permeability-field}.
...
Fig. \ref{fig:Bottom-hole-pressure-CA} shows the bottom hole pressure
history. We use this pressure history to calculate the average values
of the produced electricity divided by the consumed surplus electricity
for the injection of air over the four months of operation from October
to January 2050. The analysis of the result shows that only 1.6\%
of the total 20 MW of consumed electricity is reproduced in the compressed
air storage process, which is not significant. Moreover, the pressure
increase in the reservoir is several times higher than the hydrostatic
pressure and is localized is a small area close to the injection well.
This large average pressure increase combined with the fast pressure
increase and drop (up to 1000 bar in few days or even hours) due to
the fluctuation of the surplus wind electricity can lead to geomechanical
instabilities in the reservoir. Therefore, it is recommended to store
smaller fractions of the available surplus electricity through a single
vertical well. The tracer profile, depicted in Fig. \ref{fig:Tracer-profile-CA},
shows that after 4 months of operation the tracer has only diffused
to a radius of 700 m in the reservoir. This means that a more confined
space of a salt cavern, which offers the same storage space but does
not pose the problems of flow in a porous medium and can be a better
storage medium if it can confine a compressed gas at several hundred
bar.

\begin{figure}[H]
\includegraphics[width=8cm]{CA_p_hist}

\caption{\label{fig:Bottom-hole-pressure-CA}Bottom hole pressure history after
the injection of compressed air from October 2050 to 1st of December
2050 and storage and production onward.}
\end{figure}

\begin{figure}[H]
\includegraphics[width=14cm]{CA_tracer_harald}

\caption{\label{fig:Tracer-profile-CA}Tracer profile after 4 months of injection
and production of compressed air in the Harald West reservoir.}
\end{figure}

To understand the low energy recovery efficiency, we can consider
the pressure profile shown in Fig. \ref{fig:Pressure-profile-CA}.
The pressure in the stimulated near well region drops relatively faster
due to a high withdrawal rate. However, since the permeability of
the reservoir is low, the flow from the first 100 m of the reservoir
to the stimulated zone is a limiting factor even though the pressure
in the reservoir is still high (more than 600 bar). An analytical
model can provide a better analysis tool for the compressed air energy
storage by estimating the maximum injection and withdrawal rates.
The necessary routine for the calculation of pseudo-pressure (see
\cite{al-hussainy_application_1966}) is already written in the attached
Matlab files and can be used for the analysis of the highest potential
of a gas field for the compressed air energy storage.

\begin{figure}[H]
\includegraphics[width=14cm]{CA_pressure_harald}

\caption{\label{fig:Pressure-profile-CA}Pressure profile at the end of compressed
air storage/production in Harald West}
\end{figure}
...
alternative to the fuel storage in the Noth sea reservoirs.
\end{enumerate}
The attached simulation files offer the possibility of linking the
Aspen Plus process simulation flowsheets to the subsurface storage
models through a Matlab script, which can provide an automated solution
for investigating more subsurface energy storage scenarios and the
possible optimization of processes.
...
\addcontentsline{toc}{chapter}{\bibname}\bibliography{energy_storage}
